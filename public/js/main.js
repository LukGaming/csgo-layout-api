...
function updateWeapons () {
  ...
  for (var key in json.player.weapons) {
    ...
    if (json.player.weapons.hasOwnProperty(key)) {
      ...
      var clazz = '.rifle'
      if (type === 'pistol') clazz = '.pistol'
      else if (type === 'c4') clazz = '.c4'
      else if (type === 'knife') clazz = '.knife'
...
function tick () {
  ...
  var btime =
    json.extra.round.bomb.maxTime -
    parseInt(new Date().getTime() / 1000 - bombtime)
  var rtime =
    json.extra.round.maxTime - parseInt(new Date().getTime() / 1000 - roundtime)
